      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="space-y-8">
            ...
            {product.specifications &&
              Object.keys(product.specifications).length > 0 && (
                <div className="space-y-4">
                  <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-walnut">
                    Specifications
                  </h3>
                  <div className="grid grid-cols-1 gap-3 text-sm">
                    {Object.entries(product.specifications).map(
                      ([key, value]) => (
                        <div
                          key={key}
                          className="flex border-b border-warm-beige pb-2"
                        >
                          <span className="w-1/3 font-bold text-near-black">
                            {key}
                          </span>
                          <span className="w-2/3 text-gray-666">
                            {value as string}
                          </span>
                        </div>
                      ),
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 p-6 bg-cream/30 border border-warm-beige">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-widest text-near-black">
                      Free UK Delivery
                    </p>
        ...
        {product.reviews && product.reviews.length > 0 && (
          <div className="mt-20 pt-12 border-t border-warm-beige">
            <h3 className="text-xl font-display text-near-black mb-8">
              Customer Reviews
            </h3>
            <div className="space-y-6">
              {product.reviews.map((review) => (
                <div
                  key={review.id}
                  className="border-b border-warm-beige pb-6"
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          className={`w-3 h-3 ${star <= review.rating ? "fill-gold text-gold" : "text-gray-300"}`}
                        />
                      ))}
                    </div>
                  </div>
