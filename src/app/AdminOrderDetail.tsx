  ...
  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] sm:min-h-[500px] md:min-h-[600px] space-y-3 sm:space-y-4 px-4">
        ...
        <p className="text-gray-400 font-medium text-sm sm:text-base">Order not found in our database.</p>
        <Link to="/admin/orders" className="text-gold font-bold uppercase tracking-widest text-[9px] sm:text-[10px] hover:underline">
          Return to Ledger
        </Link>
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-5 md:gap-6">
        <div className="space-y-3 sm:space-y-4">
          ...
          >
            ...
            <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">Back to Ledger</span>
          </button>
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-display text-near-black uppercase tracking-tight">
              Order #{order.id.slice(-8).toUpperCase()}
            </h1>
            <span className={`px-2.5 sm:px-3 md:px-4 py-1 sm:py-1.5 rounded-sm border text-[8px] sm:text-[9px] md:text-[10px] font-bold uppercase tracking-[0.15em] ${statusColors[order.orderStatus]}`}>
              {order.orderStatus}
            </span>
          </div>
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 md:gap-6 text-gray-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">
            ...
            <span className="flex items-center gap-1.5 sm:gap-2">
              ...
              {order.createdAt?.toDate().toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-4">
          ...
          >
            <Printer className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" /> Print Invoice
          </button>
          <div className="relative group">
            <button className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 md:px-5 lg:px-6 py-2 sm:py-2.5 md:py-3 bg-near-black text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest hover:bg-gold transition-colors disabled:opacity-50 rounded" disabled={updating}>
            ...
            <div className="absolute right-0 top-full mt-2 w-44 sm:w-48 bg-white border border-warm-beige shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 rounded">
              {(["pending", "processing", "shipped", "delivered", "cancelled"] as Order["orderStatus"][]).map((status) => (
                <button
                  key={status}
        ...
        <div className="lg:col-span-2 space-y-6 sm:space-y-8 md:space-y-10">
          ...
          <section className="bg-white border border-warm-beige overflow-hidden rounded-lg">
            <div className="px-4 sm:px-5 md:px-6 lg:px-8 py-3 sm:py-4 md:py-5 lg:py-6 border-b border-warm-beige flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-cream/30">
              <h2 className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-near-black">Order Contents</h2>
              <span className="text-[9px] sm:text-[10px] font-bold text-gray-400 uppercase tracking-widest">{order.products.length} Items</span>
            </div>
                  ...
                  {order.products.map((p, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-2 sm:px-3 md:px-4 py-3 sm:py-4 md:py-5 lg:py-6">
                        <p className="text-xs sm:text-sm font-bold text-near-black">{p.title}</p>
                        <p className="text-[8px] sm:text-[9px] md:text-[10px] text-gray-400 uppercase font-bold tracking-widest mt-1">ID: {p.productId.slice(-8).toUpperCase()}</p>
                      </td>
                      <td className="px-2 sm:px-3 md:px-4 py-3 sm:py-4 md:py-5 lg:py-6 text-center">
                        <p className="text-[8px] sm:text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-gray-400">Qty</p>
                        <p className="text-xs sm:text-sm font-medium mt-1">{p.quantity}</p>
                      </td>
                      <td className="px-3 sm:px-4 md:px-5 lg:px-8 py-3 sm:py-4 md:py-5 lg:py-6 text-right">
                        <p className="text-[8px] sm:text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-gray-400">Price</p>
                        <p className="text-xs sm:text-sm font-bold mt-1 text-near-black">{formatCurrency(p.price)}</p>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="p-4 sm:p-5 md:p-6 lg:p-8 bg-near-black text-white">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4">
                <div>
                  <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-1">Settlement Total</p>
                  <p className="text-[10px] sm:text-xs text-gray-500 font-medium italic">Includes all applicable duties and carriage fees.</p>
                </div>
                <div className="text-right">
                  <p className="text-2xl sm:text-3xl md:text-4xl font-display font-bold leading-none">{formatCurrency(order.totalPrice)}</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white border border-warm-beige p-4 sm:p-5 md:p-6 lg:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 rounded-lg">
            <div className="flex items-center gap-3 sm:gap-4 md:gap-5 lg:gap-6">
              ...
              <div>
                <h3 className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-near-black mb-0.5 sm:mb-1">Payment Method</h3>
                <p className="text-xs sm:text-sm font-medium text-gray-600">Cash on Delivery (COD)</p>
              </div>
            </div>
        ...
        <div className="space-y-6 sm:space-y-8 md:space-y-10">
          ...
          <section className="bg-white border border-warm-beige rounded-lg overflow-hidden">
            <div className="px-4 sm:px-5 md:px-6 lg:px-8 py-3 sm:py-4 md:py-5 lg:py-6 border-b border-warm-beige bg-cream/30">
              <h2 className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-near-black">Customer Profile</h2>
            </div>
            <div className="p-4 sm:p-5 md:p-6 lg:p-8 space-y-5 sm:space-y-6 md:space-y-7 lg:space-y-8">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  <p className="text-xs sm:text-sm font-bold text-near-black">{order.customerInfo.fullName}</p>
                  <p className="text-[8px] sm:text-[9px] md:text-[10px] text-gray-400 uppercase font-bold tracking-widest mt-0.5">UID: {order.userId.slice(-8).toUpperCase()}</p>
                </div>
              </div>
              <div className="space-y-4 sm:space-y-5 md:space-y-6">
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-bold uppercase tracking-widest text-gray-400 mb-1">Comms Endpoint</p>
                    <p className="text-xs sm:text-sm font-bold text-near-black break-words">{order.customerInfo.email}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 sm:gap-4">
                  <Phone className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-walnut mt-0.5" />
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-bold uppercase tracking-widest text-gray-400 mb-1">Contact Line</p>
                    <p className="text-xs sm:text-sm font-bold text-near-black">{order.customerInfo.phone}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section className="bg-white border border-warm-beige rounded-lg overflow-hidden">
            <div className="px-4 sm:px-5 md:px-6 lg:px-8 py-3 sm:py-4 md:py-5 lg:py-6 border-b border-warm-beige bg-cream/30">
              <h2 className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-near-black">Shipping Coordinates</h2>
            </div>
            <div className="p-4 sm:p-5 md:p-6 lg:p-8 space-y-5 sm:space-y-6 md:space-y-7 lg:space-y-8">
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-bold uppercase tracking-widest text-gray-400 mb-1">Physical Address</p>
                    <p className="text-xs sm:text-sm font-bold text-near-black leading-relaxed">{order.customerInfo.address}</p>
                    <p className="text-[10px] sm:text-xs font-medium text-gray-500 mt-1 uppercase tracking-widest">{order.customerInfo.city}, {order.customerInfo.postalCode}</p>
                    <p className="text-[10px] sm:text-xs font-medium text-gold mt-0.5 uppercase tracking-widest">{order.customerInfo.country}</p>
                  </div>
                  {order.customerInfo.notes && (
                    <div className="p-3 sm:p-4 bg-gold/5 border border-gold/10 italic rounded">
                      <p className="text-[7px] sm:text-[8px] font-bold uppercase tracking-widest text-walnut mb-1 sm:mb-2">Delivery Directives</p>
                      <p className="text-[10px] sm:text-[11px] text-walnut leading-relaxed">"{order.customerInfo.notes}"</p>
                    </div>
                  )}
                </div>
              </div>
              <div className="pt-4 sm:pt-5 md:pt-6 border-t border-warm-beige space-y-3 sm:space-y-4">
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-bold uppercase tracking-widest text-gray-400 mb-1">Fulfillment Mode</p>
                    <p className="text-xs sm:text-sm font-bold text-near-black uppercase tracking-widest">Standard Carrier</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 sm:gap-4">
                  <Package className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-walnut" />
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-bold uppercase tracking-widest text-gray-400 mb-1">Packaging Status</p>
                    <p className="text-xs sm:text-sm font-bold text-near-black uppercase tracking-widest">Ready for Despatch</p>
                  </div>
                </div>
              </div>
            </div>
