  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4 p-8 text-center">
        ...
        <h3 className="text-lg font-display text-near-black">Error Loading Users</h3>
              ...
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 sm:px-6 py-12 sm:py-16 text-center">
                    <EmptyState 
                      ...
                      description={searchQuery ? "No results match your search criteria" : "No users have registered yet"}
                    />
                  </td>
                </tr>
              ) : (
                filteredUsers.map((user) => (
                  <tr key={user.uid} className="hover:bg-cream/20 transition-colors">
                    ...
                    <td className="px-4 sm:px-6 md:px-8 py-4 sm:py-5 md:py-6">
                      <span className={`inline-flex items-center gap-1 px-2 sm:px-3 py-1 text-[8px] sm:text-[9px] font-bold uppercase tracking-widest rounded-full whitespace-nowrap ${
                        user.role === 'admin' 
